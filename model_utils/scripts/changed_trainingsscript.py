...
#from torch.utils.tensorboard import SummaryWriter  # Removed: not using tensorboard
...
# Removed TensorBoard SummaryWriter initialization since tensorboard is not being used.
...
def train(loader, net, criterion, optimizer, device, debug_steps=100, epoch=-1):
    ...
    # Instead of logging to TensorBoard, we return the metrics.
    return train_loss, train_regression_loss, train_classification_loss
...
if __name__ == '__main__':
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        train_loss, train_reg_loss, train_clf_loss = train(train_loader, net, criterion, optimizer, device=DEVICE, debug_steps=args.debug_steps, epoch=epoch)
        scheduler.step()
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            # If no validation, set dummy values so we can log JSON.
            val_loss, val_regression_loss, val_classification_loss = None, None, None

        # Every epoch (or the final epoch), write metrics to a JSON file.
        metrics = {
            "epoch": epoch,
            "train": {
                "loss": train_loss,
                "regression_loss": train_reg_loss,
                "classification_loss": train_clf_loss
            },
            "validation": {
                "loss": val_loss,
                "regression_loss": val_regression_loss,
                "classification_loss": val_classification_loss
            }
        }
        # Include mAP info if computed.
        if args.validation_mean_ap and (epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1):
            metrics["validation"]["mean_average_precision"] = mean_ap
            metrics["validation"]["class_average_precision"] = class_ap

        json_file_path = os.path.join(args.checkpoint_folder, "metrics.json")
        ...
        logging.info(f"Wrote metrics to {json_file_path}")

    logging.info("Task done, exiting program.")
    # Close TensorBoard writer if it had been used (omitted here).
